...
class UploadFormDialog(QDialog):
	...
	def _init_ui(self) -> None:
		...
		self.download_template_button = QPushButton(
			icon=QIcon("./icons/download_template_icon.svg"), text="Download Template File"
		)
	...
	def load_provider_from_csv(self, file_path: str, blue_shield_provider_name: str, model_lob_value: str):
		...
		try:
			...
			self.blue_shield_id_data = (
				dataframe  # assigning the filename to the property that will transfer the file path to the table view.
			)
